...
# Compute adaptive radius based on mesh size
def compute_adaptive_radius(vertices, scale_factor=0.2):
    bbox = np.ptp(vertices, axis=0)  # Bounding box (point-to-point distance)
    avg_scale = np.mean(bbox)
    return avg_scale * scale_factor

# Adaptive normal estimation using PCA with neighborhood adjustment
def estimate_normals_pca_adaptive(vertices, min_neighbors=6, scale_factor=0.02):
    normals = np.zeros(vertices.shape)
    eigenvalues_list = np.zeros((vertices.shape[0], 3))
    kdtree = cKDTree(vertices)
    adaptive_radius = compute_adaptive_radius(vertices, scale_factor)
    
    for i, vertex in enumerate(vertices):
        neighbors = kdtree.query_ball_point(vertex, r=adaptive_radius)
...
def estimate_curvature_multi_scale(vertices, normals, scales=(0.01, 0.02, 0.03)):
    ...
    for scale_idx, radius in enumerate(scales):
        adaptive_radius = compute_adaptive_radius(vertices, scale_factor=radius)
...
def compute_dk_dn_adaptive(mesh, curvature, vertex_normals, sigma=0.01, scale_factor=0.05):
    dn_q = np.zeros(mesh.vertices.shape[0])
    dk_q = np.zeros(mesh.vertices.shape[0])
    kdtree = cKDTree(mesh.vertices)
    
    adaptive_radius = compute_adaptive_radius(mesh.vertices, scale_factor)
    vertex_area = np.full(mesh.vertices.shape[0], 1.0)
    
    for i, q in enumerate(mesh.vertices):
        neighbors = kdtree.query_ball_point(q, r=adaptive_radius)
        if len(neighbors) < 10:
            continue
